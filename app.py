        ...
        for pag in pagamentos_previstos:
            if pag.data_vencimento <= limite_7_dias:
                ...
                pagamentos_futuros_normais.append({
                    'descricao': pag.descricao,
                    'fornecedor': pag.fornecedor if pag.fornecedor else '-',
                    'pix': pag.pix if pag.pix else '-',  # Adicionar PIX
                    'tipo_pagamento': '-',
                    'valor': pag.valor,
                    'vencimento': pag.data_vencimento,
                    'status': pag.status
        ...
        for pag_serv in pagamentos_servicos_pendentes:
            if pag_serv['data_vencimento'] <= limite_7_dias:
                ...
                pagamentos_futuros_normais.append({
                    'descricao': pag_serv['descricao'],
                    'fornecedor': pag_serv['fornecedor'] if pag_serv['fornecedor'] else '-',
                    'pix': pag_serv['pix'],  # Adicionar PIX aqui também
        ...
        if pagamentos_resumo:
            ...
            for pag in pagamentos_resumo:
                ...
                data_resumo.append([
                    pag['descricao'][:25],
                    pag['fornecedor'][:15],
                    pix_paragraph,  # PIX com quebra automática (SEM corte)
                    formatar_real(pag['valor']),
                    pag['vencimento'].strftime('%d/%m/%Y'),
                    status_text
                ])
            
            # Ajustar larguras das colunas (PIX agora tem 3.5cm)
            ...
            table.setStyle(TableStyle([
                ...
                ('VALIGN', (2, 1), (2, -1), 'MIDDLE'),  # Coluna PIX centralizada verticalmente
                ('WORDWRAP', (2, 1), (2, -1), True),    # Quebra automática na coluna PIX
            ]))
            elements.append(table)
            elements.append(Spacer(1, 0.5*cm))
        ...
        if pagamentos_futuros_normais:
            ...
            for pag in pagamentos_futuros_normais:
                ...
                data_futuros.append([
                    pag['descricao'][:25],
                    pag['fornecedor'][:15],
                    pix_paragraph,  # PIX com quebra automática
                    formatar_real(pag['valor']),
                    pag['vencimento'].strftime('%d/%m/%Y')
                ])
            
            # Ajustar larguras com coluna PIX (3.5cm)
